...
public class ControlerAllocation {
	...
	public ControlerAllocation(DataBaseManager db) {
		this.db = db;
	}
	...
	public void autoAllocate() {
		try {
			...
			tableShop = db.select("SELECT * FROM shop WHERE place_id IS NULL");
			for(int i = 0; i < tableShop.size(); i++) {
				String[] ligne = tableShop.get(i); 
				listShop.add(new Shop(ligne[0], ligne[1], ligne[2], ligne[3], ligne[4], ligne[5]));
			}
			tableShop.clear();
			listShop.sort(null);
			
			listPlace = new ArrayList<Place>();
			tablePlace = db.select("SELECT * FROM place WHERE start_date IS NULL");
			for(int i = 0; i < tablePlace.size(); i++) {
				String[] ligne = tablePlace.get(i);
				listPlace.add(new Place(ligne[0], ligne[1], ligne[2], ligne[3], ligne[4], ligne[5], ligne[6], ligne[7]));
			}
			tablePlace.clear();
			listPlace.sort(null);
